...
4. Avoid duplicating tasks that already exist in the vault, via a Dataview query.
...
```
`text` is the **normalized** task text (leading dashes, trailing status annotations, and any emoji
stripped). Two small pure helpers — `parse_open_line` and `parse_completed_line` — hold the regex
grammar so they're unit-testable in isolation.
...
> These non-`On Notice` mappings are defaults chosen for sanity; confirm/adjust during spec review.

### 3. Dedup via Dataview — `vault.search_query`

Before writing, run one Dataview DQL query through the existing MCP `search_query`:
```
TABLE text, completion FROM -"zz-Templates" FLATTEN file.tasks AS t
```
Build a set of **normalized existing task texts** (lowercased, dashes/status/emoji stripped). Skip
any open or completed item whose normalized text is already present. Matching on normalized text
makes re-runs idempotent. If the query fails, log and proceed without dedup (degrade, don't abort).

### 4. Placement & rendering

- **Open items** → today's daily note, appended under a new `## Open Items` heading (distinct from
  the existing dataview-driven `## TODO`), as `- [ ] <text> <priority> <due>`.
...
- Dedup against a fake `search_query` (present → skipped; absent → kept; query failure → no dedup).
...
- Priority mapping: On Notice → `⏫` + Friday due date.
